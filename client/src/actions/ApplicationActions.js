...
export const getAppliedJob = () => async (dispatch) => {
    try {
      ...
      // backend sends { success, count, applications: [...] }
      const list = data.applications || data.allApplications || []
    
      dispatch(allAppliedJobsSuccess(list))
    } catch (err) {
      console.error('getAppliedJob error:', err)
      dispatch(
        allAppliedJobsFail(
          err.response?.data?.message || err.message || 'Failed to fetch applied jobs'
        )
      )
    }
}
